...
include_once(APP_PATH.'Home/Common/zfb/lib/alipay_submit.class.php');
include_once(APP_PATH.'Home/Common/zfb/lib/alipay_notify.class.php');
...
class ZhifubaoController extends Controller {
    ...
    echo $this->getZhifubaoData($orderInfo);
    }
    /**
     * 
     ...
     * @return string
     */
	private function getZhifubaoData($orderInfo){
		$alipay_config = include_once (APP_PATH.'Home/Common/zfb/alipay.config.php');
		//支付类型
		//必填，不能修改
		$payment_type = "1";
		//商户订单号
		$out_trade_no = $orderInfo['order_card'];
		...
		$short_title = isset($message -> short_title) ? $message -> short_title : 0;
		//订单名称
		$subject = $short_title;
		//必填
		//付款金额
    ...
    public function alipayReturn(){
    	...
    	if($verify_result) {//验证成功
    		...
    		if($_GET['trade_status'] == 'TRADE_FINISHED') {
    			...
    			$result = $order -> updateOrder(array('pay_type' => self::PAY_TYPE,'trade_no'=>$trade_no, 'order_card' => $out_trade_no, 'total_fee' => $total_fee));
